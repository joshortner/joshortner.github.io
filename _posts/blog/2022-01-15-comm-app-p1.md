...
The general idea of the application is to allow for users-managed servers. So, some users will host servers on their personal machines, while others can use the frontend to connect to a known server, create an account and, once authorized, hold conversations with other users registered on the server. It's similar to how some online video games work: Minecraft and Valheim are good examples.
