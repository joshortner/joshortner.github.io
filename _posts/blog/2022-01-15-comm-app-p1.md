...
The utils.PathExists comes from a local package that will hold general helper functions. PathExists wraps a couple other functions from Go's os package: os.Stat and os.IsNotExists. os.Stat returns a FileInfo[[7]](#go-os-fileinfo) struct describing the named file, while os.IsNotExist checks if the error returned by os.Stat indicates that the file or directory does not exist.[[1]](#go-os)[[5]](#go-os-stat)[[6]](#go-os-fileinfo) 
...
Before implementing initializeServer, I'll add a new struct to hold all of the server directories and file paths. It's called ServerDiskContext. The only directories I need right now are base and log. I'll use the Join function from Go's filepath library to generate subdirectories,[[8]](#go-filepath-join) and the updated code looks like this:
...
Now I'll implement initializeServer. For now the job of this function is simply to create the directories specified in the ServerDiskContext struct[[1]](#go-os)[^fn9].
...
Now, to implement GenerateNewLogFile. Log files will be named by converting a time stamp from Go's time package[[3]](#go-time) to a string and appending ".log". This is probably something else I'll need to revisit. Right now though I'm keeping things simple until the basic functionality is proven.
...
I'll make a new package called cli, and use Go's bufio[[4]](#go-bufio) package to read input from the terminal.
...
The very next thing I plan to do is implement an actual database. But that's for another post.
...
<a id="go-os-isnotexist">[6]</a> 
Go os IsNotExist.
https://pkg.go.dev/os#IsNotExist

<a id="go-os-fileinfo">[7]</a> 
Go os FileInfo.
https://pkg.go.dev/os#FileInfo

<a id="go-filepath-join">[8]</a> 
Go filepath Join.
